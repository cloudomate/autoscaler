...
type ChangeServerOsWithCloudInitOption struct {
	...
	// 密钥名称。  密钥可以通过密钥创建接口进行创建（请参见[创建和导入SSH密钥](https://support.g42cloud.com/api-ecs/zh-cn_topic_0020212678.html)），或使用SSH密钥查询接口查询已有的密钥（请参见[查询SSH密钥列表](https://support.g42cloud.com/api-ecs/ecs_03_1201.html) ）。

	Keyname *string `json:"keyname,omitempty"`
	// 用户ID。 说明 如果使用秘钥方式切换操作系统，则该字段为必选字段。

	Userid *string `json:"userid,omitempty"`
	// 切换系统所使用的新镜像的ID，格式为UUID。  镜像的ID可以从控制台或者参考[《镜像服务API参考》](https://support.g42cloud.com/api-ims/ims_03_0702.html)的“查询镜像列表”的章节获取。

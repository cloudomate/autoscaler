...
import (
	...
	g42cloudsdkasmodel "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/g42cloud/g42cloud-sdk-go-v3/services/as/v1/model"
	g42cloudsdkecs "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/g42cloud/g42cloud-sdk-go-v3/services/ecs/v2"
	g42cloudsdkecsmodel "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/g42cloud/g42cloud-sdk-go-v3/services/ecs/v2/model"
...
type cloudServiceManager struct {
	...
	getECSClientFunc func() *g42cloudsdkecs.EcsClient
	getASClientFunc  func() *g42cloudsdkas.AsClient
...
func (csm *cloudServiceManager) DeleteServers(serverIDs []string) error {
	...
	servers := make([]g42cloudsdkecsmodel.ServerId, 0, len(serverIDs))
	for i := range serverIDs {
		s := g42cloudsdkecsmodel.ServerId{
			Id: serverIDs[i],
		}
	...
	deleteVolume := false
	opts := &g42cloudsdkecsmodel.DeleteServersRequest{
		Body: &g42cloudsdkecsmodel.DeleteServersRequestBody{
	...
	err = wait.Poll(5*time.Second, 300*time.Second, func() (bool, error) {
		showJobOpts := &g42cloudsdkecsmodel.ShowJobRequest{
			JobId: *jobID,
		}
		...
		jobStatusEnum := g42cloudsdkecsmodel.GetShowJobResponseStatusEnum()
...
func (csm *cloudServiceManager) ListScalingInstances(groupID string) ([]g42cloudsdkasmodel.ScalingGroupInstance, error) {
	asClient := csm.getASClientFunc()
	if asClient == nil {
		return nil, fmt.Errorf("failed to list scaling groups due to can not get as client")
	}

	var scalingGroupInstances []g42cloudsdkasmodel.ScalingGroupInstance
	var startNumber int32 = 0
	for {
		opts := &g42cloudsdkasmodel.ListScalingInstancesRequest{
...
func (csm *cloudServiceManager) DeleteScalingInstances(groupID string, instanceIds []string) error {
	...
	instanceDeleteBatchRemoveInstancesOption := g42cloudsdkasmodel.GetBatchRemoveInstancesOptionInstanceDeleteEnum().YES

	opts := &g42cloudsdkasmodel.BatchRemoveScalingInstancesRequest{
		ScalingGroupId: groupID,
		Body: &g42cloudsdkasmodel.BatchRemoveInstancesOption{
			InstancesId:    instanceIds,
			InstanceDelete: &instanceDeleteBatchRemoveInstancesOption,
			Action:         g42cloudsdkasmodel.GetBatchRemoveInstancesOptionActionEnum().REMOVE,
		},
	}
...
func (csm *cloudServiceManager) IncreaseSizeInstance(groupID string, delta int) error {
	...
	addOperation := g42cloudsdkasmodel.GetScalingPolicyActionV1OperationEnum().ADD
	instanceNum := int32(delta)
	opts := &g42cloudsdkasmodel.CreateScalingPolicyRequest{
		Body: &g42cloudsdkasmodel.CreateScalingPolicyOption{
			// It's not mandatory for AS service to set a unique policy name.
			ScalingPolicyName: "g42cloudautoscaler",
			ScalingGroupId:    groupID,
			ScalingPolicyType: g42cloudsdkasmodel.GetCreateScalingPolicyOptionScalingPolicyTypeEnum().SCHEDULED,
			ScheduledPolicy: &g42cloudsdkasmodel.ScheduledPolicy{
				LaunchTime: time.Now().Add(time.Hour).UTC().Format("2006-01-02T15:04Z"),
			},
			ScalingPolicyAction: &g42cloudsdkasmodel.ScalingPolicyActionV1{
	...
	deletePolicyOps := &g42cloudsdkasmodel.DeleteScalingPolicyRequest{
		ScalingPolicyId: spID,
	}
	defer csm.deleteScalingPolicy(deletePolicyOps)

	// execute policy immediately
	executeAction := g42cloudsdkasmodel.GetExecuteScalingPolicyOptionActionEnum().EXECUTE
	executeOpts := &g42cloudsdkasmodel.ExecuteScalingPolicyRequest{
		ScalingPolicyId: spID,
		Body: &g42cloudsdkasmodel.ExecuteScalingPolicyOption{
...
func (csm *cloudServiceManager) ListScalingGroups() ([]AutoScalingGroup, error) {
	...
	// requiredState := g42cloudsdkasmodel.GetListScalingGroupsRequestScalingGroupStatusEnum().INSERVICE
	opts := &g42cloudsdkasmodel.ListScalingGroupsRequest{
...
func (csm *cloudServiceManager) transformInstanceState(lifeCycleState g42cloudsdkasmodel.ScalingGroupInstanceLifeCycleState,
	healthStatus g42cloudsdkasmodel.ScalingGroupInstanceHealthStatus) *cloudprovider.InstanceStatus {
	instanceStatus := &cloudprovider.InstanceStatus{}

	lifeCycleStateEnum := g42cloudsdkasmodel.GetScalingGroupInstanceLifeCycleStateEnum()
	...
	healthStatusEnum := g42cloudsdkasmodel.GetScalingGroupInstanceHealthStatusEnum()
	switch healthStatus {
...
func (csm *cloudServiceManager) createScalingPolicy(opts *g42cloudsdkasmodel.CreateScalingPolicyRequest) (scalingPolicyID string, err error) {
	asClient := csm.getASClientFunc()
	if asClient == nil {
...
func (csm *cloudServiceManager) executeScalingPolicy(opts *g42cloudsdkasmodel.ExecuteScalingPolicyRequest) error {
	asClient := csm.getASClientFunc()
	if asClient == nil {
		return fmt.Errorf("failed to get as client")
	}
...
func (csm *cloudServiceManager) deleteScalingPolicy(opts *g42cloudsdkasmodel.DeleteScalingPolicyRequest) error {
	asClient := csm.getASClientFunc()
	if asClient == nil {
		return fmt.Errorf("failed to get as client")
	}
...
func (csm *cloudServiceManager) getScalingGroupByID(groupID string) (*g42cloudsdkasmodel.ScalingGroups, error) {
	asClient := csm.getASClientFunc()
	opts := &g42cloudsdkasmodel.ShowScalingGroupRequest{
		ScalingGroupId: groupID,
	}
...
func (csm *cloudServiceManager) listScalingTagsByID(groupID string) (map[string]string, error) {
	asClient := csm.getASClientFunc()
	opts := &g42cloudsdkasmodel.ListScalingTagInfosByResourceIdRequest{
		ResourceType: g42cloudsdkasmodel.GetListScalingTagInfosByResourceIdRequestResourceTypeEnum().SCALING_GROUP_TAG,
		ResourceId:   groupID,
	}
...
func (csm *cloudServiceManager) getScalingGroupConfigByID(groupID, configID string) (*g42cloudsdkasmodel.ScalingConfiguration, error) {
	asClient := csm.getASClientFunc()
	opts := &g42cloudsdkasmodel.ShowScalingConfigRequest{
...
func (csm *cloudServiceManager) listFlavors(az string) (*[]g42cloudsdkecsmodel.Flavor, error) {
	ecsClient := csm.getECSClientFunc()
	opts := &g42cloudsdkecsmodel.ListFlavorsRequest{
		AvailabilityZone: &az,
	}

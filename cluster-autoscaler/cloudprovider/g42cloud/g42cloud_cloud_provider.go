...
// g42cloudCloudProvider implements CloudProvider interface defined in autoscaler/cluster-autoscaler/cloudprovider/cloud_provider.go
type g42cloudCloudProvider struct {
...
func newCloudProvider(opts config.AutoscalingOptions, do cloudprovider.NodeGroupDiscoveryOptions, rl *cloudprovider.ResourceLimiter) *g42cloudCloudProvider {
	if do.AutoDiscoverySpecified() {
		klog.Errorf("only support static discovery scaling group in g42cloud for now")
		return nil
	}
...
func (hcp *g42cloudCloudProvider) Name() string {
	return cloudprovider.G42cloudProviderName
}

// NodeGroups returns all node groups managed by this cloud provider.
func (hcp *g42cloudCloudProvider) NodeGroups() []cloudprovider.NodeGroup {
	hcp.lock.RLock()
...
func (hcp *g42cloudCloudProvider) NodeGroupForNode(node *apiv1.Node) (cloudprovider.NodeGroup, error) {
...
func (hcp *g42cloudCloudProvider) HasInstance(node *apiv1.Node) (bool, error) {
	return true, cloudprovider.ErrNotImplemented
}

// Pricing returns pricing model for this cloud provider or error if not available. Not implemented.
func (hcp *g42cloudCloudProvider) Pricing() (cloudprovider.PricingModel, errors.AutoscalerError) {
	return nil, cloudprovider.ErrNotImplemented
}

// GetAvailableMachineTypes get all machine types that can be requested from the cloud provider. Not implemented.
func (hcp *g42cloudCloudProvider) GetAvailableMachineTypes() ([]string, error) {
	return []string{}, nil
}
...
func (hcp *g42cloudCloudProvider) NewNodeGroup(machineType string, labels map[string]string, systemLabels map[string]string,
	taints []apiv1.Taint, extraResources map[string]resource.Quantity) (cloudprovider.NodeGroup, error) {
	return nil, cloudprovider.ErrNotImplemented
}

// GetResourceLimiter returns struct containing limits (max, min) for resources (cores, memory etc.).
func (hcp *g42cloudCloudProvider) GetResourceLimiter() (*cloudprovider.ResourceLimiter, error) {
	return hcp.resourceLimiter, nil
}

// GPULabel returns the label added to nodes with GPU resource.
func (hcp *g42cloudCloudProvider) GPULabel() string {
	return GPULabel
}

// GetAvailableGPUTypes returns all available GPU types cloud provider supports.
func (hcp *g42cloudCloudProvider) GetAvailableGPUTypes() map[string]struct{} {
	return availableGPUTypes
}

// GetNodeGpuConfig returns the label, type and resource name for the GPU added to node. If node doesn't have
// any GPUs, it returns nil.
func (hcp *g42cloudCloudProvider) GetNodeGpuConfig(node *apiv1.Node) *cloudprovider.GpuConfig {
	return gpu.GetNodeGPUFromCloudProvider(hcp, node)
}

// Cleanup currently does nothing.
func (hcp *g42cloudCloudProvider) Cleanup() error {
	return nil
}
...
func (hcp *g42cloudCloudProvider) Refresh() error {
	return nil
}

func (hcp *g42cloudCloudProvider) buildAsgs(specs []string) error {
	...
	for _, spec := range specs {
		if err := hcp.addNodeGroup(spec, asgs); err != nil {
			klog.Warningf("failed to add node group to g42cloud provider with spec: %s", spec)
			return err
		}
	}
...
func (hcp *g42cloudCloudProvider) addNodeGroup(spec string, asgs []AutoScalingGroup) error {
...
func (hcp *g42cloudCloudProvider) addAsg(asg *AutoScalingGroup) {
	hcp.autoScalingGroup = append(hcp.autoScalingGroup, *asg)
	hcp.cloudServiceManager.RegisterAsg(asg)
}

// BuildG42cloud is called by the autoscaler/cluster-autoscaler/builder to build a g42cloud cloud provider.
func BuildG42Cloud(opts config.AutoscalingOptions, do cloudprovider.NodeGroupDiscoveryOptions, rl *cloudprovider.ResourceLimiter) cloudprovider.CloudProvider {
